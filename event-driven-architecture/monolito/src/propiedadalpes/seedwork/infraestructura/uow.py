...
from propiedadalpes.seedwork.dominio.entidades import AgregacionRaiz
...
class UnidadTrabajo(ABC):
    ...
    def registrar_batch(self, operacion, *args, lock=Lock.PESIMISTA, repositorio_eventos_func=None, **kwargs):
        ...
        print("debug repositorio_eventos_func-->", repositorio_eventos_func)
...
def flask_uow():
    from flask import session
    from propiedadalpes.config.uow import UnidadTrabajoSQLAlchemy
    if session.get('uow'):
        return session['uow']
    else:
